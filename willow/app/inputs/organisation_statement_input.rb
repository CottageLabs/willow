class OrganisationStatementInput < NestedAttributesInput
...
protected
  ...
  def build_components(attribute_name, value, index, options)
    ...
    required  = false
    if object.required?(:organisation_nested) and index == 0
      required = true
    end
    ...
    # --- role
    role_options = RdssOrganisationRolesService.select_all_options
    field = :role
    field_name = name_for(attribute_name, index, field)
    field_id = id_for(attribute_name, index, field)
    field_value = org_statement.send(field).first
