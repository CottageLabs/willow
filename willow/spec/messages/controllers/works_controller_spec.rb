...
describe CurationConcerns::WorksController, :type => :controller do
  ...
  describe '#create' do
    ...
    it 'message payload is validated' do
      message = notification_message_for('create_work.sufia') do
        post :create, params: { work: { title: ['New Work Title'] } }
      end

      payload_json =  JSON.pretty_generate(message[:messageBody][:payload])
      puts payload_json

      expect(payload_json).to_not be_nil

      expect(@validator.fully_validate(
          file_fixture("schemas/jisc_rdss/messages/metadata/create/request_schema.json").read,
          payload_json)).to be_empty
    end
  end
end
