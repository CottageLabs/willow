  ...
  describe 'Object Rights Accesses Attribute Renderer' do
    ...
    context 'when type is an enumeration value' do
      ...
      it 'looks up and displays the label' do
        expect(render_access(value).css('strong')[0].text).to eq('Safeguarded:')
      end

      it 'displays the access statement' do
        expect(render_access(value).css('span')[0].text).to eq('Statement 1')
      end
    end
    ...
    context 'when type is not an enumeration value' do
      ...
      it 'it displays the type as directly entered' do
        expect(render_access(value).css('strong')[0].text).to eq('notInEnum:')
      end

      it 'displays the access statement' do
        expect(render_access(value).css('span')[0].text).to eq('Statement 2')
      end
    end
  end
end