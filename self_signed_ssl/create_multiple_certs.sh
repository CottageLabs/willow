...
for certificate in $(echo "$SELF_SIGNED_CERTIFICATES" | tr ";" "\n")
do
    count="${certificate//[^|]}"
    if [ "${#count}" != "3" ]; then
    ...
    if [ ! -f "$certpath" ] || [ ! -f "$keypath" ] || [ "$SELF_SIGNED_CERTIFICATES_REGENERATE" = "true" ]; then
        ...
        echo "Not re-generating $certpath and $keypath"
    fi
done
